...
import { deliveriesApi, queryKeys } from "@/lib/api";
...
export default function DeliveryDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.deliveries.detail(id) });
      setIsEditing(false);
    },
  });
  ...
  if (isLoading) {
    return (
      <Column fillWidth padding="24" horizontal="center">
        <Text variant="body-default-m">Loading delivery...</Text>
      </Column>
    );
  }
  ...
  if (!delivery) {
    return (
      <Column fillWidth padding="24" horizontal="center">
        <Text variant="body-default-m">Delivery not found</Text>
        <Button variant="secondary" onClick={() => router.push("/deliveries")}>
          Back to Deliveries
        </Button>
      </Column>
    );
    ...
    <Column fillWidth gap="24" padding="24">
      <Row vertical="center" horizontal="between">
        <Row gap="16" vertical="center">
          ...
          <Text variant="heading-default-xl">{delivery.reference}</Text>
        </Row>
        <Row gap="8">
          {isEditing ? (
            <>
            ...
            <>
              ...
              {delivery.status === "ready" && <Button variant="primary">Complete</Button>}
            </>
          )}
        </Row>
      </Row>
        ...
        {isEditing ? (
          ...
          <Grid columns={2} gap="16" m={{ columns: 1 }} fillWidth>
            ...
            <Column gap="4">
              ...
              <Text variant="body-default-m">{delivery.destination || "-"}</Text>
            </Column>
            <Column gap="4">
              <Text variant="label-default-s" onBackground="neutral-weak">
                Warehouse
              </Text>
              <Text variant="body-default-m">{delivery.warehouse?.name || "-"}</Text>
            </Column>
            <Column gap="4">
              <Text variant="label-default-s" onBackground="neutral-weak">
                Status
              </Text>
              <Text variant="body-default-m">{delivery.status}</Text>
            </Column>
            <Column gap="4">
              <Text variant="label-default-s" onBackground="neutral-weak">
                Scheduled Date
            ...
            <Column gap="4">
              ...
              <Text variant="body-default-m">{delivery.notes || "-"}</Text>
            </Column>
          </Grid>
        )}
      </Card>
